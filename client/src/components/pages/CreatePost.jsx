...
import api from '../../api/axios.js'  // Make sure this import is correct
...
function CreatePost() {
  ...
  const handleSubmit = async () => {
    if (!images.length && !content) {
      ...
      return Promise.reject(new Error('No content or images'))
    }
    setLoading(true)
    ...
    try {
      ...
      console.log('Sending form data:', { content, images, postType })
      ...
      if (data.success) {
        ...
        return Promise.reject(new Error(data.message || 'Post creation failed'))
      }
    } catch (error) {
      setLoading(false)
