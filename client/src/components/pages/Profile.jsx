...
function Profile() {
  ...
  const fetchUser = async (profileId) => {
    ...
    try {
      const { data } = await api.post(`/api/user/profile`, { profileId }, {headers: {Authorization: `Bearer ${token}`}
      })
      if(data.success){
        ...
        <div className='mt-6'>
          <div className='bg-white rounded-xl shadow p-1 flex max-w-md mx-auto'>
            {["posts", "media", "likes"].map((tab)=>(
              <button onClick={()=> setActiveTabs(tab)} key={tab} className={`flex-1 px-4 py-2 text-sm font-medium rounded-lg transition-colors cursor-pointer ${activeTab === tab ? "bg-indigo-600 text-white" : "text-gray-600 hover:text-gray-900"}`}>
