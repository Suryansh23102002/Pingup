...
function RecentMessages() {
  ...
  const fetchRecentMessages= async () => {
    try {
        ...
        if(data.success && data.message){ // Ensure data.message exists
          // FIX #1: Access data.message (singular), not data.messages
  ...
  useEffect(()=>{
    if(user){
      fetchRecentMessages()
      // FIX #2: Correctly set up and clear the interval to prevent memory leaks
